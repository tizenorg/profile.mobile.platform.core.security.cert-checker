...
// FIXME:
// Notice that some of these tests might fail if real connman DBus signal will be received,
...
BOOST_AUTO_TEST_CASE(logic_workflow) {
...
BOOST_AUTO_TEST_CASE(logic_workflow2) {
    ...
    app_t app5("app_5", "pkg_5", 100, {{"OCSP_APP_REVOKED"}});
...
BOOST_AUTO_TEST_CASE(logic_workflow3) {
    ...
    sleep(1);
    std::list<app_t> buff = get_buffer_();
    std::list<app_t> apps = {app1, app2, app3, app4, app5};
    LogDebug("Apps:");
    for (const auto &iter : apps) {
        LogDebug(iter.str());
    }
    ...
    apps = {app3, app5};
#else
    apps = {app3}; // only if popup is disabled
#endif
    BOOST_REQUIRE(buff == apps);
}
