...
static int timeout_child (int timeout)
{
    int timeout_pid;
...
static int wait_for_popup (int popup_pid)
{
    int status;
    ...
    switch (static_cast <response_e> (status)) {

    case response_e::NO_ERROR:
        LogDebug("NO_ERROR");
        return 0;
        ...
        int timeout)
{
    ...
    if(childpid == 0) { // Child process
        ...
        if (timeout > 0 && timeout_child(timeout) == -1) {
            LogError("Cannot setup timeout process");
            goto error;
        }

        if (wait_for_popup(childpid) != 0 ) {
            goto error;
        }
