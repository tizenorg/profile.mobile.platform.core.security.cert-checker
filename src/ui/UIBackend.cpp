...
bool UIBackend::createUI(const std::string &app_id, const std::string &pkg_id)
{
    ...
    err = notification_set_pkgname(m_notification, "cert-checker");
    ...
    // Set title
    char *dialogTitle = (char *) "OCSP verification failed"; //dgettext(PROJECT_NAME, "SID_PRIVILEGE_REQUEST_DIALOG_TITLE");
    ...
    // Set message
    //    char *messageFormat = (char *) "Application \%s (\%s)  OCSP verififaction has failed";  //dgettext(PROJECT_NAME, "SID_PRIVILEGE_REQUEST_DIALOG_MESSAGE");

    char tmpBuffer[BUFSIZ];
    int ret = std::snprintf(tmpBuffer, sizeof(tmpBuffer), "Application %s (%s)  OCSP verififaction has failed.", app_id.c_str(), pkg_id.c_str());
    ...
    ret = snprintf(tmpBuffer, sizeof(tmpBuffer), "%s,%s",
                   "Keep application",
