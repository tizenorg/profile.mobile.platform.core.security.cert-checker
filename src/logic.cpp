...
{
    // Add pkgmgr callback
    if (package_manager_create(&m_request) != PACKAGE_MANAGER_ERROR_NONE) {
        // This function can return "success" or "invalid param only"
        LogError("package_manager_create error");
        return INVALID_PARAM;
    }

    LogDebug("register installedApp event callback start");
    if (0 != package_manager_set_event_cb(m_request, Logic::pkg_manager_callback, this)) {
