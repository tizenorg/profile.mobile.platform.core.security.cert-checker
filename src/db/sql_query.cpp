...
void SqlQuery::get_app_list(std::list<app_t> &apps_buffer)
{
    ...
    getChainsCommand->BindInt32(107, iter_app.check_id);
        ...
        while (getChainsCommand->Step()) {
            ...
            chain.chain_id = getChainsCommand->GetColumnInt32(0);
            LogDebug("Found chain (" << chain.chain_id << ") for " << iter_app.str());

            SqlConnection::DataCommandAutoPtr getCertsCommand =
                    m_connection->PrepareDataCommand(DB_CMD_GET_CERTS);
            getCertsCommand->BindInt32(110, chain.chain_id);

            // Add found certs to chain
            while (getCertsCommand->Step()) {
                chain.certificates.push_back(getCertsCommand->GetColumnString(0));
                LogDebug("Found certificate (" << chain.certificates.size() << ") in chain no. " << chain.chain_id);
            }
            iter_app.certificates.push_back(chain);
        }
    }
}
