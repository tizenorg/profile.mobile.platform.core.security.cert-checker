...
namespace {
    ...
    const char *DB_CMD_UPDATE_URL =
            //"REPLACE INTO ocsp_urls(url, date) VALUES(?102, ?103) WHERE issuer=?101;";
            "UPDATE ocsp_urls SET url=?102, date=?103 WHERE issuer=?101;";
    ...
    const char *DB_CMD_REMOVE_CERTS =
            "DELETE FROM certs_to_check WHERE check_id=?107;";

    const char *DB_CMD_REMOVE_APP =
            "DELETE FROM to_check WHERE check_id=?107;";
...
{
    if (m_connection != NULL || m_inUserTransaction != false) {
...
bool SqlQuery::check_if_app_exists(const app_t &app)
{
    ...
    if (getCheckIDCommand->Step()) {
        LogDebug(app.str() << "exists in database");
        return true;
    }
    LogDebug(app.str() << "doesn't exist in database");
    return false;
}
...
bool SqlQuery::add_app_to_check_list(const app_t &app)
{
    ...
    if (get_check_id(app, check_id)) {
        ...
        for (std::vector<std::string>::const_iterator iter = app.certificates.begin();
                iter != app.certificates.end(); ++iter) {
...
void SqlQuery::remove_app_from_check_list(const app_t &app)
{
    ...
    if (get_check_id(app, check_id)) {
        LogDebug("Found check id: " << check_id << " for app " << app.str());
        //Remove certificates from cert_to_check table
        SqlConnection::DataCommandAutoPtr removeCertsCommand =
                        m_connection->PrepareDataCommand(DB_CMD_REMOVE_CERTS);
        removeCertsCommand->BindInt32(107, check_id);
        removeCertsCommand->Step();
        LogDebug("Removed certificates for app: " << app.str());
        ...
        removeAppCommand->BindInt32(107, check_id);
...
void SqlQuery::get_apps(std::list<app_t> &list)
{
    // This function will fill buffer with check_id, app_id, pkg_id, uid and verified fields -
    // it leaves certificates list empty. Use get_certs(Buffer &buffer) to fill it in.
...
void SqlQuery::get_certs(std::list<app_t> &list)
{
    for (std::list<app_t>::iterator iter = list.begin(); iter != list.end(); ++iter) {
...
void SqlQuery::get_app_list(std::list<app_t> &buffer)
{
    get_apps(buffer);
    get_certs(buffer);
}
