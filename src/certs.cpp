...
void Certs::get_certificates (app_t &app, std::list<url_t> &ocsp_urls)
{
    ...
    if (app.app_id == TEPM_APP_ID) {
        LogDebug("Temporaty app_id. Searching for apps in package.");
 ...
 */
void Certs::search_app (app_t &app, std::list<url_t> &ocsp_urls)
{
    DIR *dp;
    struct dirent *entry;
    const char *pkg_path = tzplatform_mkpath(TZ_USER_APP, app.pkg_id.c_str());
...
void Certs::find_app_signatures (app_t &app, const std::string &app_path, std::list<url_t> &ocsp_urls)
{
