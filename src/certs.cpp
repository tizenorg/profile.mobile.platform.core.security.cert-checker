    ...
    for (auto iter = signature_files.begin(); iter != signature_files.end(); iter++) {
        ...
        try {
            ...
            for (auto cert_iter = certs.begin(); cert_iter != certs.end(); cert_iter++ ){
                ...
                chain.certificates.push_back(app_cert);
        ...
        if (!chain.certificates.empty()) {
            app.certificates.push_back(chain);
            LogDebug("Certificates chain added to the app");
        }
    }
}

Certs::ocsp_response_t Certs::check_ocsp_chain (const certificates_chain &chain)
{
    ...
    collection.clear();
    LogDebug("Chain size: " << chain.certificates.size());
    for (auto &iter : chain.certificates) {
        ValidationCore::CertificatePtr p_cert(new ValidationCore::Certificate(
                ValidationCore::Certificate(iter, ValidationCore::Certificate::FORM_BASE64)));
    ...
    ValidationCore::CertificateList certs_list = collection.getCertificateList();
    ...
    CKM::CertificateShPtrVector empty;
    ...
    int status;
    ret = m_ckm->ocspCheck(vect_ckm_chain, status);
    if (ret != CKM_API_SUCCESS) {
...
Certs::ocsp_response_t Certs::check_ocsp (const app_t &app)
{
    ...
    for (auto &iter : app.certificates) {
